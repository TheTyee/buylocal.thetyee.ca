...
App.CardView = Backbone.View.extend({
    ...
    template: _.template("<li data-card='<%= EntryId %>' class='card individual-letter'><%= DateCreated %><br /><a href='https://twitter.com/share?url=http://localhost:4000/letters/show/<%= EntryId %>&text=Thank you, <%= Field652 %>!&via=TheTyee&hashtags=bcbuylocal' target='_blank'><i class='iconic fa fa-twitter-square fa-2x'></i></a><a href='https://www.facebook.com/dialog/send?app_id=441246329398694&name=A Buy Local Thank You to <%= Field652 %>&description=test&link=http://localhost:4000/letters/show/<%= EntryId %>&redirect_uri=http://buylocal.thetyee.ca/fbr.php' target='_blank'><i class='iconic fa fa-facebook-square fa-2x'></i></a><a class='email-share' target='_blank' href='mailto:?subject=Buy Local&body=I thought you might like to see this! Read it at http://localhost:4000/letters/show/<%= EntryId %>'><i class='iconic fa fa-envelope fa-2x'></i></a><br /><br />Dear <%= Field652 %>, <br /><br /><%= Field655 %><br /> <a href='/letters/#show/<%= EntryId %>'>Permalink</a></li>"),
    render: function() {
...
App.CardDetailView = Backbone.View.extend({
    ...
    template: _.template("<div data-card='<%= EntryId %>' class='card-detail individual-letter'><%= DateCreated %><br /><a href='https://twitter.com/share?url=http://localhost:4000/letters/show/<%= EntryId %>&text=Thank you, <%= Field652 %>!&via=TheTyee&hashtags=bcbuylocal ' target='_blank'><i class='iconic fa fa-twitter-square fa-2x'></i></a><a href='https://www.facebook.com/dialog/send?app_id=441246329398694&name=A Buy Local Thank You to <%= Field652 %>&description=test&link=http://localhost:4000/letters/show/<%= EntryId %>&redirect_uri=http://buylocal.thetyee.ca/fbr.php' target='_blank'><i class='iconic fa fa-facebook-square fa-2x'></i></a><a class='email-share' target='_blank' href='mailto:?subject=Buy Local&body=I thought you might like to see this! Read it at http://localhost:4000/letters/show/<%= EntryId %>'><i class='iconic fa fa-envelope fa-2x'></i></a><br /><br />Dear <%= Field652 %>, <br /><br /> <%= Field655 %></div><br /><a href='/letters/#show/<%= EntryId %>'>Permalink</a><br /><a class='show-list' href='#'>Back</a>"),
    render: function() {
        this.$el.show();
        this.$el.html(this.template(this.model.attributes));
        return this;
