...
App.CardView = Backbone.View.extend({
    ...
    template: _.template("<li data-card='<%= EntryId %>' class='card'><%= DateCreated %><br /><a href='https://twitter.com/share?url=http://localhost:4000/letters/show/<%= EntryId %>&text=Thank you, <%= Field652 %>!&via=TheTyee&hashtags=bcbuylocal' target='_blank'>Twitter</a><br /><a href='/letters/#show/<%= EntryId %>'>Permalink</a><br />Dear <%= Field652 %><br /><%= Field655 %></li>"),
    render: function() {
...
App.CardDetailView = Backbone.View.extend({
    ...
    template: _.template("<div data-card='<%= EntryId %>' class='card-detail'><%= DateCreated %><br /><a href='https://twitter.com/share?url=http://localhost:4000/letters/show/<%= EntryId %>&text=Thank you, <%= Field652 %>!&via=TheTyee&hashtags=bcbuylocal ' target='_blank'>Twitter</a><br /><a href='/letters/#show/<%= EntryId %>'>Permalink</a><br />Dear <%= Field652 %>:<br /> <%= Field655 %></div><br /><a class='show-list' href='#'>Back</a>"),
    render: function() {
        this.$el.show();
        this.$el.html(this.template(this.model.attributes));
        return this;
